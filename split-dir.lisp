...
(defparameter pname (pathname-name *load-pathname*))


(defun print-help ()
  (format t "~&~A usage:~%" pname)
  (format t "    ~A [-h|--help]~%" pname)
  (format t "    ~A [--preview] max-size src-dir dst-dir~%" pname)
  (format t "max-size :== number ['K'|'M'|'G']~%"));;print-help
    ...
    (let ((m nil) (f nil))
      ...
      (if (eq :error (second test))
          ...
          (assert (and (= m (second test))
                       (= f (third test))))))));;test-mantissa-factor
  ...
  (do ((list list (cdr list))
    ...
    (when (< max-size (+ chunk-size (cdr (car list))))
      ...
      (setf chunk '()
            chunk-size 0))
    (push (car list) chunk)
    (incf chunk-size (cdr (car list)))));;split-groups

(defun hardlink (src dst) (ext:run-program "ln" :arguments (list "-f" src dst)))
(defun symlink  (src dst) (ext:run-program "ln"
                            :arguments (list "-f" "-s" src dst)))
(defun move     (src dst) (ext:run-program "mv" :arguments (list src dst)))
