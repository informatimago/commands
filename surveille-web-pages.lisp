...
(setf CUSTOM:*MISC-ENCODING*         charset:utf-8 ; useless
      CUSTOM:*TERMINAL-ENCODING*     charset:utf-8 ; useless
      CUSTOM:*DEFAULT-FILE-ENCODING* charset:utf-8) ; for make-pipe
...
;;;;
;;;; $Id: surveille-web-pages,v 1.8 2005/12/08 02:04:43 pupjb Exp $
;;;;
...
(defpackage "DE.PMSF.MD5"
  ...
  (:export
   ...
   #:md5sum-sequence #:md5sum-stream #:md5sum-file)) ;;DE.PMSF.MD5

(in-package "DE.PMSF.MD5")
...
where a is the intended low-order byte and d the high-order byte."
  `(the ub32 (logior (ash ,d 24) (ash ,c 16) (ash ,b 8) ,a))) ;;assemble-ub32
...
(defmacro with-md5-round ((op block) &rest clauses)
  (loop for (a b c d k s i) in clauses
     ...
     (return `(progn ,@result)))) ;;with-md5-round
...
registers A, B, C and D."
  `(simple-array (unsigned-byte 32) (4))) ;;md5-regs

(defmacro md5-regs-a (regs)
  `(aref ,regs 0)) ;;md5-regs-a

(defmacro md5-regs-b (regs)
  `(aref ,regs 1)) ;;md5-regs-b

(defmacro md5-regs-c (regs)
  `(aref ,regs 2)) ;;md5-regs-c

(defmacro md5-regs-d (regs)
  `(aref ,regs 3)) ;;md5-regs-d

(defconstant +md5-magic-a+ (assemble-ub32 #x01 #x23 #x45 #x67)
  "Initial value of Register A of the MD5 working state.") ;;+md5-magic-a+
(defconstant +md5-magic-b+ (assemble-ub32 #x89 #xab #xcd #xef)
  "Initial value of Register B of the MD5 working state.") ;;+md5-magic-b+
(defconstant +md5-magic-c+ (assemble-ub32 #xfe #xdc #xba #x98)
  "Initial value of Register C of the MD5 working state.") ;;+md5-magic-c+
(defconstant +md5-magic-d+ (assemble-ub32 #x76 #x54 #x32 #x10)
  "Initial value of Register D of the MD5 working state.") ;;+md5-magic-d+
...
The resulting MD5 message-digest is returned as an array of
sixteen(unsigned-byte 8) values.  Calling `update-md5-state' after a call to
...
(defun md5sum-file (pathname)
  ...
  (with-open-file (stream pathname :element-type '(unsigned-byte 8))
    (md5sum-stream stream))) ;;md5sum-file
...
according to the test suite in Appendix A.5 of RFC 1321") ;;+rfc1321-testsuite+
...
(defconstant +other-testsuite+
  ...
  "AList of test input strings and stringified message-digests
according to my additional test suite") ;;+other-testsuite+
...
(defun test-with-testsuite (testsuite)
  (loop for count from 1
     ...
     (format *trace-output*
       ...
       failed (1- count))
     (return (zerop failed)))) ;;test-with-testsuite

#+md5-testing
(defun test-rfc1321 ()
  (test-with-testsuite +rfc1321-testsuite+)) ;;test-rfc1321

#+md5-testing
(defun test-other ()
  (test-with-testsuite +other-testsuite+)) ;;test-other
...
(defun data-mime-type (data)
  (let ((oldenc CUSTOM:*DEFAULT-FILE-ENCODING*))
    (unwind-protect
         (progn
           (setf CUSTOM:*DEFAULT-FILE-ENCODING* (ext:make-encoding
                                                 :charset charset:iso-8859-1
                                                 :line-terminator :unix))
           (multiple-value-bind (io in out) (ext:run-program "file"
                                              :arguments (list "-ib" "-")
                                              :input :stream
                                              :output :stream)
             (close io)
             (ignore-errors
               (write-sequence (ext:convert-string-from-bytes
                                data CUSTOM:*DEFAULT-FILE-ENCODING*) out))
             (close out)
             (prog1 (read-line in) (close in))))
      (setf CUSTOM:*DEFAULT-FILE-ENCODING* oldenc))))
...
(defun get-resource (resource)
  (setf (resource-data resource)
        (ecase  (resource-kind resource)
          ((:page)
           (with-open-stream (in (ext:make-pipe-input-stream
...
(defun send-notice (task changes)
  (with-open-stream (msg (ext:make-pipe-output-stream
    ...
    (let ((boundary
           (multiple-value-bind (se mi ho da mo ye)
             ...
             (format nil "~A-~8,'0X-~4,'0D~2,'0D~2,'0D~2,'0D~2,'0D"
                     (with-open-stream
                         (in (ext:run-shell-command "hostname -f"
                                                    :output :stream))
                       (read-line in))
                     (random #.(expt 2 32)) ye mo da ho mi se))))
