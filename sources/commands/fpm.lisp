...
(defun e (command &rest args)
  (uiop:run-program (cons command args) :wait nil :input nil :output :string))
        ...
        (labels ((unambiguous (packages)
                 ...
                 (cppv (designator)
                   ...
                   (multiple-value-bind (all category name1 name2 version)
                       (match *portage-cppv-regexp* designator)
                     ;; (print (list all category name1 name2 version))
                     (if all
                         (let ((category (match-string designator category))
                               (name1    (match-string designator name1))
                               (name2    (match-string designator name2))
                               (version  (match-string designator version)))
                 ...
                 (cpv (designator)
                   ...
                   (multiple-value-bind (all category name version)
                       (match *portage-cpv-regexp* designator)
                     ;; (print (list all category name version))
                     (if all
                         (let ((category (match-string designator category))
                               (name     (match-string designator name))
                               (version  (match-string designator version)))
                           (make-cpv category name version))
                         (error "Invalid package designator: ~S" package-designator))))
                 (pv (designator)
                   ...
                   (multiple-value-bind (all name version)
                       (match *portage-pv-regexp* designator)
                     ;; (print (list all name version))
                     (if all
                         (let ((name     (match-string designator name))
                               (version  (match-string designator version)))
                           (mapcan (compose ensure-list parse-portage-package-designator)
