...
status: in-progress - framework done (006 fixed, help-option exits); 45/64 built commands expose -h/-v/-V in the framework table (+ commands/memo handle their own); all remaining lack-the-trio commands have a dedicated fix issue, so homogenize there
---
...
### Ad-hoc-parser migrations — DONE

Migrated the hand-rolled parsers to the framework:
- `bin-to-c-array`, `llen`: replaced the private `process-arguments`/
  `meta-options` parser with `(options …)` (bin-to-c-array keeps its -d/-o/-x
  radix and -bt/-st type options), collecting file operands via the
  undefined-argument handler and a small `input-files` generator (honours
  `--` and `-`=stdin).
- `clean-name`: `-f/--list-file` is now a real repeatable framework option;
  name operands collected via the handler, preserving the "Unknown option"
  error and `--`.
- `when`: dropped the `(member "-h" …)` check.
- `cookie`: `-l/--list-files` and `-f/--file` are framework options now;
  `cookie-loop` and `cookie-merge` wired too.
- `commands` (meta-dispatcher) and `memo` (verb CLI) intentionally keep their
  own argument handling — forcing `parse-options` would break command
  pass-through / verb dispatch. They gained explicit `-v`/`-V` clauses instead;
  `commands` keeps its richer `-V`. (These two won't show in the option-table
  audit by design.)

### Remaining — all have a dedicated fix issue, homogenize there

cddb-to-tag (030), check-surface (031), clean-bd-archive (013), cookie-diff
(023), edit-comments-of-ogg (044), hexbin (043), html-make-image-index (040),
lc (041), menu (011), pic-resize (033), rstuml (034), split-dir (035),
substitute (036), surveille-host (037), surveille-web-pages (014),
svn-locate-revision (015), text (042). Several of these are stubs (010/011/
013/014/015) that need a `command` form and `main` first.
